...
var getModulePath = function getModulePath(packageName, cwd) {
  ...
  console.info('module =>', _util2.default.inspect(module));
...
function pack(packageName, cwd) {
  ...
  return getModulePath(packageName, cwd).then(function (filePath) {
    ...
    console.info('pack: ' + JSON.stringify({ filePath: filePath, destDir: destDir, prebuiltPackagePath: prebuiltPackagePath, zipFrom: zipFrom, zipTo: zipTo, nodeModulePath: nodeModulePath }));
    return mkdirp(destDir)
    //.then(x => ncp(filePath, prebuiltPackagePath))
    .then(function () {
      console.info('file copied, starting 7z of ' + zipFrom + ' to ' + zipTo);
      return (0, _es7z.add7z)(zipTo, zipFrom, { exePath: exePath }).progress(function (files) {
        return console.info('progress: 7z\'ing files ' + JSON.stringify(files));
      }).then(function () {
...
function install(packageName, cwd) {
  if (!packageName) return _bluebird2.default.reject(new Error('packageName is required.'));
  var zipFrom = getZipTo(packageName, cwd);
  var packagePath = getPrebuiltPackage(packageName, cwd);
  var installPath = cwd ? _path2.default.join(cwd, 'node_modules', packageName) : _path2.default.join('node_modules', packageName);
  console.info('extracting package at ' + zipFrom + ' to ' + installPath + '...');
  return (0, _es7z.extractFull7z)(zipFrom, installPath, { exePath: exePath }).progress(function (files) {
    return console.info('progress: extracting files ' + JSON.stringify(files));
  }).then(function () {
