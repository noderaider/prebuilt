...
function executeGyp(commands, _ref) {
  var _ref$msvs_version = _ref.msvs_version;
  var msvs_version = _ref$msvs_version === undefined ? 2015 : _ref$msvs_version;
  var cwd = _ref.cwd;

  return new _bluebird2.default(function (resolve, reject) {
    var _cp$spawnSync = _child_process2.default.spawnSync(_path2.default.resolve(__dirname, '..', 'node_modules', '.bin', 'node-gyp' + (isWin ? '.cmd' : '')), [].concat(_toConsumableArray(commands), ['--msvs_version=' + msvs_version]), { cwd: _path2.default.resolve(cwd), encoding: 'utf8' });
...
function pack(packageName) {
  ...
  if (!packageName) return _bluebird2.default.reject(new Error('packageName is required.'));
  return getModulePath(packageName, root).then(function (filePath) {
    ...
    var gypOpts = { msvs_version: 2015, cwd: _path2.default.resolve(zipFrom) };
    return gypClean(gypOpts).then(function () {
      ...
      return mkdirp(destDir).then(function () {
        ...
        .then(function () {
          ...
          console.error(_util2.default.inspect(err), '7-zip error occurred');
          throw err;
        });
      });
    });
